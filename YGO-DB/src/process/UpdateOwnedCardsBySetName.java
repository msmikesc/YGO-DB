...
public class UpdateOwnedCardsBySetName {
	...
	public void run() throws SQLException, IOException {
		...
		ArrayList<OwnedCard> cards = SQLiteConnection.getAllOwnedCardsWithoutSetCode();
		...
		for (OwnedCard card : cards) {
			...
			if(!newSetName.equals(card.setName)) {
				...
				CardSet setIdentified = SQLiteConnection.getFirstCardSetForCardInSet(card.cardName, newSetName);
				...
				ArrayList<SetMetaData> metaData = SQLiteConnection.getSetMetaDataFromSetName(newSetName);
				...
				try{
					SQLiteConnection.UpdateOwnedCardByUUID(card);
					count++;
				}
				catch(Exception e) {
					e.printStackTrace();
